...
class Booking extends MU_Controller {
    ...
    */
    ...
    public function view_booking_package($bkID, $bkType){
        ...
        if($this->input->post('btnviewsubmit')){
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(count($passengercheckbox) && $passengercheckbox[0] != ""){
                  $updatebooking['bk_total_people'] = $this->input->post('bkTotalPeople');
                  $this->saveChangePassenger($passengercheckbox, $bkID, $bookingpassid['passID']);
                  if(count($extraservicecheckbox) && $extraservicecheckbox[0] != ""){
                    $this->saveChangeExtraservice($bkID, $this->input->post('pkconprice'), count($passengercheckbox), $extraservicecheckbox);
                  }  
                }else{
                   $updatebooking['bk_total_people'] = $this->input->post('bkTotalPeople');
                }

                if($this->priceBooking != 0) $updatebooking['bk_pay_price'] = $this->priceBooking;

                if($this->is_money($updatebooking['bk_pay_price'])){
    ...
    /****************/
    public function saveChangePassenger($passengercheckbox, $bkID, $passID){
    ...
    /****************/
    public function saveChangeExtraservice($bkID, $pkconprice, $totalpeole, $extraservicecheckbox){
        $extraservices = array();
        ...
        $this->priceBooking = $prices * ($totalpeole + 1);

        $newextrservices = serialize($newextrservices);
    ...
    */
    ...
    public function add_morepassenger($bkID, $passID, $bkType){
        ...
        if($result){
            ...
            redirect('booking/view_booking_package/'.$bkID.'/'.$bkType);
        }
    }
    /****************/
