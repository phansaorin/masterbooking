              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                $this->load->view('munich_admin', $data);
                  ...
                  if($this->is_money($this->input->post('salePrice')) AND $this->is_money($this->input->post('purchasePrice')) AND $checkavailableday[0] != ""){
                    $act_insert = $this->mod_extra_products->createActivities($insert_product);
                    if($act_insert) $date_available =  $this->mod_extra_products->insertDateTime($insert_calendar);
                    if($date_available) $records_conjection =  $this->mod_extra_products->insertActDateTime($act_insert, $date_available);
