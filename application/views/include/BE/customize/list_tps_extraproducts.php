...
<table class="table table-striped table-hover table-bordered" style="font-size: 12px;">
    <thead>
    <tr>
        <th><input type="checkbox" name="checkbox_all_cus_eptps" class="checkbox_all_cus_eptps check_checkbox" data-id=".tbl_bodytpsep<?php echo $tpsID; ?>" checked="true"></th>
        <th>Name</th>
        <th>From Date</th>
        <th>To Date</th>
        <th>Purchase($)</th>
    ...
    <tbody class="ep_cus_body tbl_bodytpsep<?php echo $tpsID; ?>">
        <?php 
            if(isset($extraproduct_cus)){
                foreach ($extraproduct_cus as $eptps) {
        ?>
                <tr class="real_ep_cus remove<?php echo $eptps['ep_id']; ?>">
                    ...
                    <td><span class="icon-list-alt cusadd clickDetailep" data-url="<?php echo base_url(); ?>customize/epdetail/tps/<?php echo $tpsID; ?>/<?php echo $this->uri->segment(3); ?>/<?php echo $eptps['ep_id']; ?>" data-toggle='modal' data-target='.modalDetials' title="Details"></span></td>
                </tr>
        <?php
                }
            }
